# Cloud Handoff — moving a strategy between this computer and the user's cloud machine

Trigger: the user asks to send a strategy to their cloud machine or pull one back to this computer — typed in chat, or sent by the desktop app's buttons, whose fixed messages are:
...
- **NEVER start, pause, resume or schedule trading on either side**, and never clear a HALT. The strategy arrives as a backtest-only draft; going live is the user's own action on the destination (`AGENTS.md` › Deployment redline).
- **NEVER write to `control/`, `lib/`, `manager/`, `runtime/` or `VERSION` on the destination** (step 4a only reads).
...
- **NEVER write the SSH key or certificate outside the workspace.** Use `tmp/cloud-handoff/` and delete it at the end (step 8). Not `~/.ssh`.
- **NEVER use the `blave` MCP tools or SSH for anything but a handoff the user asked for in this conversation** — not to fetch data, not to "check on" the cloud machine, not because a local data call failed. Never read or print the app's MCP configuration.
...
| `os_type` is `linux` and `get_ssh_access` returned user `blaveagent` | "Handoff isn't supported on this kind of cloud machine yet." Stop — do not improvise paths or log in as root. |
...
Compare the local `VERSION` with the remote one. If they differ, say so before going on ("this computer is on `<a>`, the cloud machine on `<b>`") and continue — a different `lib/` can change the numbers, and the final report repeats it. **Do not update either side yourself.** If the destination backtest then fails inside `lib/` (ImportError, AttributeError, a missing function), that is the "could not run" state: name the version gap as the likely cause and how to close it — cloud: tell the cloud agent 「更新」; desktop: update the app.
...
- local → cloud: `ssh <SSH_OPTS> blaveagent@<host> "cd /opt/blave-agent/workspace && python3 strategies/<name>/strategy.py"` — the ONE place `cd … &&` is allowed: a single quoted remote command, and the strategy must run from the workspace root.
