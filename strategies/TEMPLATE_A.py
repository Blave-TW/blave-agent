...
START         = "2024-01-01"
END           = None
FEE           = 0.0005            # PER SIDE (one-way) — VERIFIED for this symbol/venue, never 0, never copied unchecked (strategy-code.md › Key Rules)
...
# ── compute_signals ───────────────────────────────────────────────────────────
# Returns pd.Series: positive=long, negative=short (swap only — MARKET="spot" is long-only), 0=flat, nan=hold.
...
def compute_signals(df):
    ...
    # Exits (stop / take-profit / trailing / time) → lib.exits.apply_exits(..., trigger="intrabar" | "close"), never a hand-written loop
    if MARKET == "spot":
        signal = signal.clip(lower=0.0)   # spot cannot short — live clamps to flat, so must the backtest
    return signal
