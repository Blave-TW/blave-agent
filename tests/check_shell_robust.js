...
if (!process.versions.electron) {
  ...
  ok("③ app.js:掃描鈕走確認框 → submitMessage(不覆寫 viewing:chatViewing 在雲端視角本來就回 env:cloud + strategy)、resolve 回合序號;回合開始 / 結束三處都叫 rpRobSync(就地改鈕,不重畫);每輪 turnSeq++",
    /onOk: \(\) => submitMessage\(t\("rob\.msgScan", \{ name \}\)\)\.then\(\(ok\) => resolve\(ok \? turnSeq : false\)\)/.test(app) && !/viewing/.test(app.slice(app.indexOf("function rpRobAsk"), app.indexOf("function rpRobSync"))) && (app.match(/rpRobSync\(\);/g) || []).length === 3
