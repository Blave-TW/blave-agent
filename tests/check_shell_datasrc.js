...
(async () => {
  ...
  const handlers = [...main.matchAll(/ipcMain\.handle\("(datasrc-[a-z]+)",\s*\((\w+)[^)]*\)\s*=>\s*\(([^\n]*)\n/g)];
  t("四支 datasrc IPC 都在,而且每一支都先過 fromOurPage(e)", handlers.length === 4 && handlers.every((m) => m[2] === "e" && /^fromOurPage\(e\)/.test(m[3])));
