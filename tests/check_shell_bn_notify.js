...
// 標題 / 內文、P1 才亮 Dock 紅點、真的交給系統才回 true、failed 要留 log。**不加任何執行期鉤子**(這份檢查也釘住這一點)。
...
{ const mk = (o = {}) => { const made = [], logs = []; let badge = null;
    ...
    const ctx = { Notification: FakeN, tmLabels: o.labels || { key_ipTitle: "IP 換了", key_ipBody: "新的 IP 是 {ip}", key_rejTitle: "金鑰被拒", key_rejSameIpBody: "b1", key_rejUnknownBody: "b2", key_permTitle: "交易權限沒了", key_permBody: "b3", key_wdTitle: "提領被打開了", key_wdBody: "b4" },
  ...
  let x = mk(); let r = x.call({ reason: "WITHDRAW_ENABLED", level: "P1", ip: null });
  t("提領被打開(P1):真的 show、標題內文對、回 true、亮 Dock 紅點", r === true && x.made.length === 1 && x.made[0].shown && x.made[0].a.title === "提領被打開了" && x.made[0].a.body === "b4" && x.badge() === "1");
  ...
  x = mk({ focused: true }); x.call({ reason: "WITHDRAW_ENABLED", level: "P1" });
  t("視窗在前景:不亮紅點(人就在看)", x.made[0].shown && x.badge() === null);
  x = mk(); t("四種 reason 都有對應的字;表外的 reason 不發、回 false", ["IP_CHANGED", "KEY_REJECTED", "REJECTED", "TRADING_LOST", "WITHDRAW_ENABLED"].every((k) => x.call({ reason: k, level: "P2" }) === true) && x.call({ reason: "constructor" }) === false && x.call({ reason: "NOPE" }) === false && x.call(null) === false);
  x = mk({ labels: {} }); t("字還沒交過來 → 不發、回 false(binance_link 不會記成已通知,下一輪再試)", x.call({ reason: "WITHDRAW_ENABLED", level: "P1" }) === false && x.made.length === 0);
  x = mk({ supported: false }); t("系統不支援通知 → 不發、回 false", x.call({ reason: "WITHDRAW_ENABLED", level: "P1" }) === false && x.made.length === 0);
  x = mk(); x.call({ reason: "WITHDRAW_ENABLED", level: "P1" }); x.made[0].emit("failed", {}, "not signed");
  t("failed:留一行 log(含哪一種)、從存活集合拿掉", x.logs.length === 1 && /binance WITHDRAW_ENABLED failed: not signed/.test(x.logs[0]) && x.ctx.p1Alive.size === 0);
  x = mk(); x.call({ reason: "WITHDRAW_ENABLED", level: "P1" }); x.made[0].emit("click");
