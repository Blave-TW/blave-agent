...
(async () => {
  ...
  t("提領開著 → 照收,不擋也不提醒(Wei 2026-09-22 拍板 MVP 不查提領)", classify(ok200({ enableWithdrawals: true }), "futures").code === "OK" && classify(ok200({ enableWithdrawals: true }), "spot").ok === true
    && classify(ok200({ enableWithdrawals: true, ipRestrict: false }), "any").code === "NO_IP_RESTRICT");
  ...
  t("提領那一格缺席或不是 boolean → 仍不下結論(要不要拿掉待 Wei 決定)", (() => { const b = ok200().body; delete b.enableWithdrawals; const r = classify({ status: 200, body: b }, "futures"); return r.ok === false && r.code === "UNKNOWN"; })()
    && classify(ok200({ enableWithdrawals: "false" }), "futures").ok === false);
  t("提領開著且交易關著 → TRADING_DISABLED(擋的是交易權限)", classify(ok200({ enableWithdrawals: true, enableFutures: false, enableSpotAndMarginTrading: false }), "futures").code === "TRADING_DISABLED");
  ...
  t("market any(連接畫面):現貨或合約至少開一個就收,細節帶哪個沒開;兩個都沒開才 TRADING_DISABLED;提領開著照收", (() => { const a = classify(ok200({ enableFutures: false }), "any"), b = classify(ok200({ enableSpotAndMarginTrading: false }), "any");
    return a.ok && a.detail.futures === false && a.detail.spot === true && b.ok && b.detail.spot === false && classify(ok200({ enableFutures: false, enableSpotAndMarginTrading: false }), "any").code === "TRADING_DISABLED" && classify(ok200({ enableWithdrawals: true }), "any").code === "OK"; })());
  ...
  // MVP 不查提領(Wei 拍板):連接與重查都不看那一格
  t("重查時提領開著 → 沒有 verdict、表上也沒有這個 reason", !("WITHDRAW_ENABLED" in VERDICT_LEVEL) && classify(ok200({ enableWithdrawals: true }), "any").ok === true);
  t("連接與重查:提領開著都照收", await (async () => { const http = async () => ok200({ enableWithdrawals: true });
    const a = await check({ apiKey: "K", secret: "S", market: "any", http }), r = await recheck({ apiKey: "K", secret: "S", market: "any", http });
    return a.ok === true && a.code === "OK" && r.ok === true; })());
  t("重查仍然要求四個欄位都是 boolean(提領那格缺席 / 不是 boolean → 不下結論)", classify(ok200({ enableWithdrawals: undefined }), "any").code === "UNKNOWN" && classify(ok200({ enableWithdrawals: "false" }), "any").code === "UNKNOWN");
