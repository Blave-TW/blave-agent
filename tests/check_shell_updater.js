...
(async () => {
  ...
  t("選單列:新版在等的時候選單多一行(圖示旁不加小點),而且會觸發重畫(進 trayKey)", /updateWaiting\(\) \? \[\{ label: tmLabels\.updateReady/.test(mainSrc) && !/\.setTitle\(/.test(mainSrc) && !/cloudUpdateWaiting/.test(mainSrc) && /const key = live \? \[[^\]]*updateWaiting\(\) \? tmLabels\.updateReady : ""[^\]]*\]\.join\("\|"\)/.test(mainSrc));
