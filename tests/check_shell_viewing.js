...
t("用戶的訊息不進 argv(同機 `ps` 看得到;聊天貼 key 是支援的流程):走 stdin", (() => { const i = mainSrc.indexOf('path.join(REPO, "runtime", "agent_turn.py")'), args = mainSrc.slice(i, mainSrc.indexOf("], { env, cwd: WS })", i)).replace(/\/\/.*$/gm, ""); return i > 0 && !/\bmessage\b/.test(args.replace(/"[^"]*"/g, "")) && /"--message-stdin", "--", sessionId,\s*$/.test(args) && /child\.stdin\.end\(message\)/.test(mainSrc); })());
