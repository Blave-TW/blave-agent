...
(async () => {
  ...
  // Binance 金鑰重查出事(複查 F-1 + 「trKeyBad 沒有測試」):單送不出去的那幾種熄綠點;提領被打開照樣下得了單,綠點留著、用中性短詞
  { const live = { alive: true, report: rep() }, withV = (reason, fn) => { globalThis.CXF = { bn: { verdict: reason ? { reason } : null } }; try { return fn(); } finally { delete globalThis.CXF; } };
    ok("金鑰出事(IP 換了 / 交易權限沒了 / 被拒)→ 這台電腦那格不亮綠點;雲端那格不受影響", ["IP_CHANGED", "TRADING_LOST", "KEY_REJECTED", "REJECTED"].every((r) => withV(r, () => cell("local", live) === "real,false,," && trKeyBad("local") === true && trKeyBad("cloud") === false && trKeyWord("local") === "cx.failShort")));
    ok("提領被打開:還下得了單 → 綠點留著,短詞是 cx.keyAttn 不是串接失敗", withV("WITHDRAW_ENABLED", () => cell("local", live) === "real,true,," && trKeyBad("local") === false && trKeyWord("local") === "cx.keyAttn"));
    ok("沒有 verdict / CXF 還沒載入 → 不算出事", withV(null, () => trKeyBad("local") === false && trKeyWord("local") === null) && trKeyBad("local") === false && cell("local", live) === "real,true,,"); }
